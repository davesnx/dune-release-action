...
- Run `dune-release` directly when it's on PATH, falling back to `opam exec -- dune-release` only when it isn't. `opam` is no longer a hard requirement: it's checked only for that fallback [#7](https://github.com/davesnx/dune-release-action/issues/7) [@davesnx](https://github.com/davesnx)
